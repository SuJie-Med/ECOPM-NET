...
class Conv_64F(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, num_classes=5, neighbor_k=3, batch_size=4, shot_num=1):
        ...
        self.classifier = dSE_Metric(neighbor_k=neighbor_k, batch_size=batch_size, shot_num=shot_num)  # 1*num_classes
...
class dSE_Metric(nn.Module):
    ...
    def cosine_distance_Batch(self, mean1, cov1, mean2, cov2):
        ...
        cosine_dis = trace_dis + maha_product + matrix_det - mean1.size(2)
    ...
    def cal_MML_similarity(self, input1_batch, input2_batch):
        ...
        for i in range(self.batch_size):
            ...
            # pixel level covariance similarity and channel level cosine similarity between a query set and a support set
            ...
            # Using Fusion Layer to fuse two parts
            kl_sim_soft2 = torch.cat(( inner_sim_l, inner_sim_p), 1)
    ...
    def forward(self, x1, x2):
        ...
        Similarity_list, Q_S_List = self.cal_dSE_similarity(x1, x2)
